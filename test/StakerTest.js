...
describe("when stake", function () {
	...
	it("stake fail when balance < amount or amount = 0 ", async () => {
		await expectRevert(
			stakerContract.connect(account1).stake(ethers.utils.parseEther("300")),
			"Please stake more in your card!"
		);
		await expectRevert(
			stakerContract.connect(account1).stake(ethers.utils.parseEther("0")),
	...
	it("stake success", async () => {
		...
		expect(await stakerContract.stakeOf(account1.address)).to.be.equal(
		...
		expect(await stakerContract.getTotalStakedAmount()).to.be.equal(
			ethers.utils.parseEther("30")
		);
		// account2 stake
		await rewardTokenContract
		...
		expect(await stakerContract.stakeOf(account2.address)).to.be.equal(
	...
	describe("when claim reward", function () {
		...
		describe("check the calculation for it when the setting of reward rate success", async () => {
			beforeEach(async () => {
				...
				await stakerContract
					.connect(account2)
					.stake(ethers.utils.parseEther("200"));
			});
			it("check if new rate is set correctly", async () => {
				expect(await stakerContract.getRewardRate()).to.be.equal(20000);
			});
			it("check the total amount of the staked tokens", async () => {
				expect(await stakerContract.getTotalStakedAmount()).to.be.equal(
					ethers.utils.parseEther("300")
				);
			});
